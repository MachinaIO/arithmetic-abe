#!/usr/bin/env python3
"""
Memory Usage Analyzer for Arithmetic ABE Logs
...
def analyze_memory_usage_from_string(log_output):
    ...
    pattern_with_message = re.compile(
        r".*?mxx::utils\s*:\s*(.*?)\s*\|\|\s*Current physical/virtual memory usage:\s*(\d+)\s*\|\s*(\d+)"
    )

    # Pattern for lines with just numbers
    pattern_numbers = re.compile(r"(\d+)\s*\|\s*(\d+)")
    ...
    df["physical_percentage_increase"] = (
        df["physical_memory_change"] / df["physical_memory"].shift(1) * 100
    ).fillna(0)
    df["virtual_percentage_increase"] = (
        df["virtual_memory_change"] / df["virtual_memory"].shift(1) * 100
    ).fillna(0)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Memory Usage Analyzer for Diamond-IO Logs"
    )
    parser.add_argument("--log-file", help="Path to an existing log file to analyze")
    parser.add_argument(
        "command",
        nargs=argparse.REMAINDER,
        help="Command to run (if not using --log-file)",
    )
